...
A Local First Auth mini app living at `apps/hobbies` in this workspace, served under
`/hobbies/` on the shared domain. Scaffolded from `templates/mini-app-starter` by
`pnpm new-app` — signup/login, REST API, SQLite (D1), and real-time WebSocket updates
are already wired.

## Everyday commands

From this directory (`apps/hobbies`):
...
- The worker serves under the subpath — in dev the API is at
  `http://localhost:<worker-port>/hobbies/api`, not `/api` (Hono `basePath`).
