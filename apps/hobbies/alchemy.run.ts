 ...
 * - R2 Bucket for chalk-drawing photos
...
const app = await alchemy('home-hobbies-mini-app', {
...
/**
 * R2 Bucket
 * Photos attached to chalk-drawing sessions (full + thumb per photo). CORS
  ...
  cors: [
    {
      allowed: {
        methods: ['PUT'],
        origins: [ALLOWED_PRODUCTION_ORIGIN, 'http://localhost:5176'],
...
// OpenAI key for the muse. Optional on purpose: a key-less deploy ships the
// curated-pool fallback (see server/src/muse.ts) instead of failing.
...
export const worker = await Worker('worker', {
  ...
  bindings: {
    ...
    ...(hasR2Creds
      ...
      : {}),
    // Muse — bound only when the key is in .env (pool fallback otherwise).
    ...(hasOpenAIKey ? { OPENAI_API_KEY: alchemy.secret.env.OPENAI_API_KEY } : {}),
  ...
  ...(hasRealOrigin
    ? {
        routes: [
          `${new URL(ALLOWED_PRODUCTION_ORIGIN).host}/hobbies/*`,
        ],
      }
    : {}),
  url: true,
