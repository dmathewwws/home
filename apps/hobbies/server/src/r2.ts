/**
 * R2 chalk-photo storage helpers, following the dweb-camp-cascadia photos app
...
export async function uploadUrlFor(env: Env, key: string): Promise<string> {
  ...
  return `/hobbies/api/dev-upload/${key}`
}
...
export async function purgeImgCache(requestUrl: string, keys: string[]): Promise<void> {
  ...
  await Promise.all(
    keys.map((key) =>
      cache.delete(new URL(`/hobbies/api/img/${key}`, requestUrl).toString()).catch(() => false),
    ),
  )
}
