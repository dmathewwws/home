...
```bash
# Dev (local host D1)
pnpm wrangler d1 execute home-dev-db --local --command "UPDATE users SET is_admin = 1 WHERE did = 'did:key:z...';"

# Production (remote host D1)
pnpm wrangler d1 execute home-prod-db --remote --command "UPDATE users SET is_admin = 1 WHERE did = 'did:key:z...';"
```
