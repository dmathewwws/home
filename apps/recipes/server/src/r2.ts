/**
 * R2 photo storage helpers, following the dweb-camp-cascadia photos app
...
export async function uploadUrlFor(env: Env, key: string): Promise<string> {
  ...
  return `/recipes/api/dev-upload/${key}`
}
...
export async function purgeImgCache(requestUrl: string, keys: string[]): Promise<void> {
  ...
  await Promise.all(
    keys.map((key) =>
      cache.delete(new URL(`/recipes/api/img/${key}`, requestUrl).toString()).catch(() => false),
    ),
  )
}
