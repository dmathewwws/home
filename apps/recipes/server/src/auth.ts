/**
 * Request auth for the shared household box.
 *
 * Every app-data endpoint (reads included) requires a member: a valid
 * Local First Auth JWT whose DID maps to a users row that is not blocked
 * and has `is_member` set (admins are implicitly members). Membership is
 * granted from the host console's admin UI, which writes `users.is_member`
 * directly through its D1 binding to this app's database.
 */
...
export async function requireMember(c: Ctx, db: Database, profileJwt: unknown): Promise<User> {
  ...
  if (!user.isMember && !user.isAdmin) {
    throw new AuthError('Members only: ask a household admin to let you in', 403)
  }
  return user
}

/**
 * Row-level guard: the row's author, or any admin.
 */
export function requireOwnerOrAdmin(user: User, createdBy: string): void {
  if (user.did !== createdBy && !user.isAdmin) {
    throw new AuthError('Unauthorized: not yours to remove', 403)
  }
}
