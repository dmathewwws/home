...
export interface User {
  ...
  isAdmin: boolean
  // Household gate: membership is granted from the host console's admin UI
  isMember: boolean
}

export type AppEventListener = (type: string, data: unknown) => void
...
interface AuthContextType {
  ...
  /** Subscribe to app broadcasts (recipe-*, reflection-*). Returns unsubscribe. */
  subscribeToEvents: (listener: AppEventListener) => () => void
}
...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  // Small event bus so screens can refetch on broadcasts without their own sockets
  const listenersRef = useRef<Set<AppEventListener>>(new Set())
  const subscribeToEvents = useCallback((listener: AppEventListener) => {
  ...
  const handleMessage = useCallback((type: string, data: unknown) => {
    for (const listener of listenersRef.current) listener(type, data)
  }, [])
